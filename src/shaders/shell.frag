...
void main() {
	...
	float thisBladeDecimal = layerHeight / randomHeight;


	//Cylindrical discard
	if ((layerIndex > 0) && USE_CYLINDRICAL_SHELLS) { //Layer 0 must be flat.
	...
	vec3 shellColour = mix(BASE_COLOUR.rgb, SHELL_COLOUR.rgb, layerDecimal) * layerDecimal;
	fragColour = vec4(mix(skyColour.rgb, shellColour.rgb, distScaling), 1.0f);
